...
const Main = ({
    ...
    pres?.forEach((block, idx) => {
      ...
      if (!block.parentNode.classList.contains("code-block-wrapper")) {
        ...
        tryBtn.onclick = () => {
          ...
          localStorage.setItem("tryThisCode", codeText);
  ...
  const getChapterButtonLabel = (currentGroup) => {
    if (!currentGroup || selectedTechnology === "GIT") return null;
 ...
 const handleChapterAction = () => {
  if (selectedTechnology === "GIT") return;
  ...
  if (quizNum) {
    ...
    window.open(`/exam/${selectedTechnology.toLowerCase()}/exam${examNum}`, "_blank");
  }
};
 ...
 const handleModalProceed = () => {
  ...
  return (
    <div className="main-body" ref={mainBodyRef}>
                ...
                {subItemIndex === activeGroup.subItems.length - 1 && (
                  <div className="nav-buttons">
                    ...
                    {selectedTechnology !== "GIT" && (
                      <button onClick={handleChapterAction} className="action-btn">
      ...
      {showModal && selectedTechnology !== "GIT" && (
