...
import {
  FaJava, FaPython, FaHtml5, FaDatabase,
  FaChalkboardTeacher, FaBookOpen, FaUserCheck, FaUsers,
  FaInfinity, FaRocket,
  FaGithub, FaLinkedin, FaFileAlt, FaLightbulb, FaReact, FaCloud,
  FaProjectDiagram, FaCss3Alt, FaJs, FaCertificate, FaHandsHelping,
  FaLaptopCode, FaGlobe
} from "react-icons/fa";
  ...
  return (
    ...
    <div className="home-container">
      ...
      <section className="career-support-section">
        <motion.div className="career-support-container" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: false, amount: 0.3 }} transition={{ duration: 0.8 }}>
          <div className="career-support-content">
            <h2 className="section-heading">Career Acceleration Path</h2>
            <motion.div className="timeline-container" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: false }}>
              {careerSupportSteps.map((step, index) => (
                <motion.div key={index} className="timeline-item" variants={itemVariants}>
                  <div className="timeline-icon-container"><div className="timeline-icon">{step.icon}</div></div>
                  <div className="timeline-text"><h3>{step.title}</h3><p>{step.description}</p></div>
                </motion.div>
              ))}
            </motion.div>
            <motion.button className="contact-advisor-btn" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
      ...
      <section ref={coursesSectionRef} className="cards-container-section">
        ...
        <motion.div className="cards-container" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: false, amount: 0.2 }}>
          {courses.map((course, index) => (
            <motion.div
              key={index}
              className="course-card"
              variants={itemVariants}
              whileHover={{ y: -10, scale: 1.03, boxShadow: "0px 15px 30px rgba(0,0,0,0.2)" }}
      ...
      <section className="features-section">
        ...
        <motion.div className="features-container" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: false, amount: 0.2 }}>
      ...
      <section className="testimonials-section">
        ...
        <div className="testimonials-container">
          <AnimatePresence mode="wait">
            <motion.div key={currentTestimonial} className="testimonial-card" variants={itemVariants} initial="hidden" animate="visible" exit="exit">
              <p className="quote">"{testimonials[currentTestimonial].quote}"</p>
              <p className="author">- {testimonials[currentTestimonial].name}, <span>{testimonials[currentTestimonial].course}</span></p>
            </motion.div>
          </AnimatePresence>
        </div>
      </section>
      ...
      <AnimatePresence>
        {modalIsOpen && (
          <Modal isOpen={modalIsOpen} onRequestClose={closeModal} contentLabel="Course Detail Modal" className="modal-content" overlayClassName="modal-overlay" closeTimeoutMS={300}>
