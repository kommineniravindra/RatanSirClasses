            ...
            <div
              className={`navbar-item ${
                (item.type === 'dropdown'
                  ...
                  : item.name === selectedTechnology || (item.name === "Home" && !selectedTechnology)
                ) ? "active" : ""
              }`}
              onClick={() => item.type === 'dropdown' ? handleDropdownToggle(item.name) : handleSelection(item.name)}
            >
              <div className="navbar-content">
                {item.icon}
                <span>
                  {item.type === 'dropdown' 
                    ? item.subItems.find(sub => sub.name === selectedTechnology)?.name || item.name
                    : item.name
                  }
                </span>
                {item.type === 'dropdown' && <FaAngleDown className={`dropdown-arrow ${openDropdown === item.name ? "open" : ""}`} />}
              </div>
            </div>
